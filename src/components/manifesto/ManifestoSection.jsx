...
const ManifestoSection = ({
  ...
  const renderContent = content => {
    if (Array.isArray(content)) {
      return content.map((section, index) => <div key={index} className="mb-16">
          {section.subtitle && <h3 className="text-3xl font-serif mb-6">{section.subtitle}</h3>}
          {Array.isArray(section.text) ? <div className="space-y-4">
              {section.text.map((paragraph, idx) => <p key={idx}>{paragraph}</p>)}
            </div> : <p>{section.text}</p>}
          {section.source && <p className="text-sm text-gray-500 italic mt-4">
    ...
    duration: 0.8
      ...
      <div className="max-w-4xl mx-auto text-center">
        ...
        y: 0
          ...
          {source && <p className="text-sm text-gray-500 italic mt-16">
              Source: {source}
            </p>}
        </motion.div>
      </div>
